...
@section('title', 'Edit Pemadaman')

@section('content')
<div class="container-fluid">
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
        <h1 class="h3 mb-0 text-gray-800">Edit Pemadaman</h1>
    </div>

    <div class="card shadow mb-4">
        <div class="card-body">
            <form action="{{ route('pemadaman.update', $pemadaman->id) }}" method="POST">
                @csrf
                @method('PUT')
                    ...
                    <select class="form-control @error('wilayah_id') is-invalid @enderror" 
                        ...
                        @foreach($wilayahs as $wilayah)
                            <option value="{{ $wilayah->id }}" 
                                {{ old('wilayah_id', $pemadaman->wilayah_id) == $wilayah->id ? 'selected' : '' }}>
                ...
                <div class="form-group mb-3">
                    ...
                    <input type="date" class="form-control @error('tanggal') is-invalid @enderror" 
                           ...
                           value="{{ old('tanggal', $pemadaman->tanggal->format('Y-m-d')) }}" required>
                    @error('tanggal')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                    ...
                    <div class="col-md-6">
                        <div class="form-group mb-3">
                            ...
                            <input type="time" class="form-control @error('jam_mulai') is-invalid @enderror" 
                                   ...
                                   value="{{ old('jam_mulai', substr($pemadaman->jam_mulai, 0, 5)) }}" required>
                            @error('jam_mulai')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                    ...
                    <div class="col-md-6">
                        <div class="form-group mb-3">
                            ...
                            <input type="time" class="form-control @error('jam_selesai') is-invalid @enderror" 
                                   ...
                                   value="{{ old('jam_selesai', substr($pemadaman->jam_selesai, 0, 5)) }}" required>
                            @error('jam_selesai')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                ...
                <div class="form-group mb-3">
                    ...
                    <select class="form-control @error('jenis_pemadaman') is-invalid @enderror" 
                        ...
                        <option value="terencana" 
                            {{ old('jenis_pemadaman', $pemadaman->jenis_pemadaman) == 'terencana' ? 'selected' : '' }}>
                            Terencana
                        </option>
                        <option value="mendadak" 
                            {{ old('jenis_pemadaman', $pemadaman->jenis_pemadaman) == 'mendadak' ? 'selected' : '' }}>
                            Mendadak
                        </option>
                    </select>
                ...
                <div class="form-group mb-3">
                    ...
                    <select class="form-control @error('status') is-invalid @enderror" 
                        ...
                        <option value="scheduled" 
                            {{ old('status', $pemadaman->status) == 'scheduled' ? 'selected' : '' }}>
                            Terjadwal
                        </option>
                        <option value="ongoing" 
                            {{ old('status', $pemadaman->status) == 'ongoing' ? 'selected' : '' }}>
                            Sedang Berlangsung
                        </option>
                        <option value="completed" 
                            {{ old('status', $pemadaman->status) == 'completed' ? 'selected' : '' }}>
                            Selesai
                        </option>
                        <option value="cancelled" 
                            {{ old('status', $pemadaman->status) == 'cancelled' ? 'selected' : '' }}>
                            Dibatalkan
                        </option>
                    </select>
                    @error('status')
                ...
                <div class="form-group mb-3">
                    ...
                    <textarea class="form-control @error('alasan') is-invalid @enderror" 
                              id="alasan" name="alasan" rows="4" required>{{ old('alasan', $pemadaman->alasan) }}</textarea>
                    @error('alasan')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
